...
def _sale_edit_allowed(db: Session, sale: ProductSale, user: AuthUser | None = None) -> tuple[bool, str]:
    ...
    # Как у визитов: суперадмин/техспец — без окна дней (пока период открыт).
    if user is not None and (UserRole.ADMIN_SUPER in user.roles or UserRole.TECHSPEC in user.roles):
        return True, ""
    days = edit_window_days(db)
    if not within_edit_window(sale, days):
        return False, (
            f"Для админов редактирование доступно только в течение {days} дн. с даты создания "
            "(параметр «Окно редактирования» в настройках студии). "
            "Суперадмин может править, пока период ЗП не закрыт; техспец — всегда (в закрытом периоде с подтверждением)."
        )
    return True, ""
