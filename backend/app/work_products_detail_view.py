...
def work_profit_explanation(work: WorkForInventory, details: dict[str, Any]) -> list[str]:
    ...
    if kind == WorkKind.KIT:
        ...
        lines = [
            "цена − себестоимость − ЗП мастера — формула профита по заказу (снимок на момент записи).",
            "Себестоимость — материал + доп. расходы.",
            "ЗП мастера — сумма полей «Работа» (master_pay) по заготовкам; бонус за смешку сейчас не учитывается.",
            "ЗП студии: если есть сумма с клиента — сумма − себестоимость − ЗП мастеров; "
            "если суммы нет (в т.ч. «в наличие») — студия = −ЗП мастеров, т.к. ЗП мастеров идёт из фонда студии.",
        ]
        if work.scope == WorkScope.CUSTOM_ORDER and work.amount_from_client is not None and float(work.amount_from_client or 0) > 0:
            lines.append(
                f"На заказ указана сумма с клиента ({work.amount_from_client} ₽) — "
                "она используется в формуле студии."
            )
        ...
        if work.scope == WorkScope.IN_STOCK:
            lines.append(
                "В режиме «в наличие» маржа с продажи учитывается отдельно при продаже комплекта; "
                "на этапе производства ЗП мастеров списывается с фонда студии (студия = −ЗП мастеров)."
            )
        return lines
